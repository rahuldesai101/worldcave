...
export class EventHandlerManager implements AppModule {
  ...
  private setupMobileMenu(): void {
    ...
    document.getElementById('mobileMenuSearch')?.addEventListener('click', () => {
      this.closeMobileMenu();
      (document.getElementById('searchBtn') as HTMLButtonElement | null)?.click();
    });

    document.getElementById('mobileMenuAi')?.addEventListener('click', () => {
      this.closeMobileMenu();
      const trigger = document.querySelector<HTMLButtonElement>(
        '#aiAssistantMount button, [data-ai-assistant-trigger]'
      );
      trigger?.click();
    });

    document.getElementById('mobileMenuFindings')?.addEventListener('click', () => {
      this.closeMobileMenu();
      const trigger = document.querySelector<HTMLButtonElement>('.intel-findings-badge');
      trigger?.click();
    });

    document.getElementById('mobileMenuLayers')?.addEventListener('click', () => {
      this.closeMobileMenu();
      const toggle = document.querySelector<HTMLButtonElement>(
        '.deckgl-layer-toggles-header, .layer-toggles-header, [data-layer-toggle-open]'
      );
      if (toggle) {
        toggle.click();
      } else {
        // Fallback: scroll to / surface the layer panel on the map.
        document.querySelector('.deckgl-layer-toggles, .layer-toggles')
