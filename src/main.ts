...
if (urlParams.get('settings') === '1') {
  ...
  app
    ...
    .then(() => {
      ...
      // Mount AI assistant trigger into the header (sign-in gated W.A.V.E.)
      import('./components/AiAssistantWidget')
        .then((m) => m.initAiAssistantWidget())
        .catch((e) => console.warn('AI assistant widget failed to load', e));
      import('./components/ProximitySandboxWidget')
        .then((m) => m.initProximityWidget())
        .catch((e) => console.warn('Proximity sandbox widget failed to load', e));
    })
    .catch(console.error);
}
