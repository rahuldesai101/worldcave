...
export class App {
  ...
  constructor(containerId: string) {
    ...
    // Expose live state to the AI assistant (W.A.V.E.) so it can build a
    // real-time snapshot of what the dashboard is currently showing.
    try { (window as unknown as { __worldcaveState?: AppContext }).__worldcaveState = this.state; } catch { /* ignore */ }
  ...
  public async init(): Promise<void> {
    ...
    // Pro launch banner intentionally disabled (user request).
    void showProBanner;
    this.updateConnectivityUi();
    ...
    // Findings badge is available on mobile too (header-right is now visible
    // on phones in a compact form, so users keep access to intel findings).
