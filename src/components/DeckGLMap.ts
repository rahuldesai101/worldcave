...
import { subscribeProximityRenderState, getProximityRenderState } from '@/services/proximity-map-bridge';
import { circlePolygon } from '@/utils/geo-circle';
...
export class DeckGLMap {
  ...
  constructor(container: HTMLElement, initialState: DeckMapState, options: DeckGLMapOptions = {}) {
    ...
    subscribeProximityRenderState(() => this.debouncedRebuildLayers());
  ...
  private buildLayers(): LayersList {
    ...
    // Proximity Sandbox — private asset overlay, danger-zone rings, and
    // pulse markers for live threats that fall inside any user zone.
    const px = getProximityRenderState();
    const enabledSets = px.sets.filter((s) => s.enabled && s.assets.length);
    if (enabledSets.length) {
      type ZoneRow = { polygon: [number, number][]; rgb: [number, number, number] };
      type AssetRow = { position: [number, number]; rgb: [number, number, number]; label: string };
      const zones: ZoneRow[] = [];
      const assets: AssetRow[] = [];
      for (const s of enabledSets) {
        const rgb = hexToRgb(s.color);
        for (const a of s.assets) {
          const radius = a.radiusKmOverride ?? s.defaultRadiusKm;
          zones.push({ polygon: circlePolygon(a.lat, a.lon, radius, 48), rgb });
          assets.push({ position: [a.lon, a.lat], rgb, label: a.label });
        }
      }
      layers.push(new PolygonLayer<ZoneRow>({
        id: 'proximity-zones',
        data: zones,
        getPolygon: (d) => d.polygon,
        getFillColor: (d) => [d.rgb[0], d.rgb[1], d.rgb[2], 36],
        getLineColor: (d) => [d.rgb[0], d.rgb[1], d.rgb[2], 180],
        getLineWidth: 1.5,
        lineWidthUnits: 'pixels',
        stroked: true, filled: true, pickable: false,
      }));
      layers.push(new ScatterplotLayer<AssetRow>({
        id: 'proximity-assets',
        data: assets,
        getPosition: (d) => d.position,
        getRadius: 5,
        radiusUnits: 'pixels',
        getFillColor: (d) => [d.rgb[0], d.rgb[1], d.rgb[2], 230],
        getLineColor: [255, 255, 255, 230],
        lineWidthUnits: 'pixels',
        getLineWidth: 1.5,
        stroked: true, pickable: true,
      }));
      if (px.activeThreats.length) {
        const sevColor = (sev: string): [number, number, number] =>
          sev === 'critical' ? [220, 38, 38] : sev === 'high' ? [239, 68, 68] : sev === 'medium' ? [245, 158, 11] : [59, 130, 246];
        type HitRow = typeof px.activeThreats[number];
        layers.push(new ScatterplotLayer<HitRow>({
          id: 'proximity-threat-hits-ring',
          data: px.activeThreats,
          getPosition: (d) => [d.lon, d.lat],
          getRadius: 14,
          radiusUnits: 'pixels',
          stroked: true, filled: false,
          getLineColor: (d) => { const c = sevColor(d.severity); return [c[0], c[1], c[2], 220]; },
          getLineWidth: 2,
          lineWidthUnits: 'pixels',
          pickable: false,
        }));
        layers.push(new ScatterplotLayer<HitRow>({
          id: 'proximity-threat-hits-dot',
          data: px.activeThreats,
          getPosition: (d) => [d.lon, d.lat],
          getRadius: 5,
          radiusUnits: 'pixels',
          getFillColor: (d) => { const c = sevColor(d.severity); return [c[0], c[1], c[2], 255]; },
          getLineColor: [255, 255, 255, 230],
