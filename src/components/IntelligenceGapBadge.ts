...
export class IntelligenceFindingsBadge {
  ...
  constructor() {
    ...
    this.dropdown.style.zIndex = '2147483200';
    ...
    if (this.enabled) {
      ...
      document.addEventListener('wm:header-dropdown-open', (e) => {
        ...
        if (detail?.id !== 'intel-findings' && this.isOpen) {
          this.closeDropdown();
        }
      });
      this.mount();
  ...
  private toggleDropdown(): void {
    ...
    if (willOpen) {
      document.dispatchEvent(new CustomEvent('wm:header-dropdown-open', { detail: { id: 'intel-findings' } }));
    }
    this.isOpen = willOpen;
    document.body.classList.toggle('wm-header-dropdown-active', this.isOpen);
  ...
  private updateDropdownPosition(): void {
    ...
    const dropdownWidth = this.dropdown.offsetWidth || 380;
    ...
    this.dropdown.style.top = `${Math.max(margin, rect.bottom + margin)}px`;
  }

  private closeDropdown(): void {
    if (!this.isOpen) return;
    this.isOpen = false;
    document.body.classList.remove('wm-header-dropdown-active');
