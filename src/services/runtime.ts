...
const DEFAULT_CLOUD_FUNCTIONS_URL = `https://${'durlyzdoyzgudcoamail'}.supabase.co/functions/v1`;
const CLOUD_FUNCTIONS_URL = ENV.VITE_SUPABASE_URL
  ? `${normalizeBaseUrl(ENV.VITE_SUPABASE_URL)}/functions/v1`
  : DEFAULT_CLOUD_FUNCTIONS_URL;
const DEFAULT_WEB_API_URL = 'https://api.worldmonitor.app';
const LOVABLE_API_PROXY_FUNCTION = 'wm-api-proxy';
...
function isLovableHostedWebHost(hostname: string): boolean {
...
export function getConfiguredWebApiBaseUrl(): string {
  ...
  // Lovable published/preview hosts do not deploy this repository's Vercel
  // /api functions. If the SPA calls same-origin /api there, hosting correctly
  // falls back to index.html, which panels interpret as unavailable data. Route
  // those builds through the Cloud edge proxy instead; it preserves the same
  // /api path contract while calling the canonical data API server-to-server.
  if (isLovableHostedWebHost(hostname) && CLOUD_FUNCTIONS_URL) {
    return `${CLOUD_FUNCTIONS_URL}/${LOVABLE_API_PROXY_FUNCTION}`;
  }
...
function isAllowedRedirectTarget(url: string): boolean {
  try {
    ...
    const cloudFunctionsOrigin = CLOUD_FUNCTIONS_URL ? new URL(CLOUD_FUNCTIONS_URL).origin : '';
    const isCloudProxy = Boolean(cloudFunctionsOrigin)
      && parsed.origin === cloudFunctionsOrigin
      && parsed.pathname.endsWith(`/${LOVABLE_API_PROXY_FUNCTION}`);
    return ALLOWED_REDIRECT_HOSTS.test(parsed.origin) || parsed.hostname === 'localhost' || isCloudProxy;
  } catch {
    return false;
  }
}
