...
function dashboardHtmlOutputPlugin(): Plugin {
  return {
    ...
    generateBundle(_options, bundle) {
      ...
      if (!dashboardEntry) {
        throw new Error('[vite] expected dashboard HTML entry index.html before emitting dashboard.html');
      }

      const [, dashboardHtml] = dashboardEntry;
      if (typeof dashboardHtml.source === 'string') {
        dashboardHtml.source = deferDashboardStylesheetLinks(dashboardHtml.source, bundle);
      }

      // Keep index.html for static hosts (including Lovable publish) while also
      // emitting dashboard.html for the production rewrite contract used by the
      // canonical deployment. Removing index.html made published builds appear
      // incomplete because the static host had no root HTML entry to serve.
