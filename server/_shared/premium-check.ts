...
export async function isCallerPremium(request: Request): Promise<boolean> {
  ...
  if (authHeader?.startsWith('Bearer ')) {
    ...
    // Payments are intentionally disabled while we let every signed-in user
    // try the Pro feature set. ANY valid Clerk bearer is treated as premium.
    // Re-introduce the role / Dodo-tier checks below when monetisation is
    // turned back on.
    return true;
  }
  return false;
