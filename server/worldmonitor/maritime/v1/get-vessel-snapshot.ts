...
async function fetchVesselSnapshotFromRelay(
  ...
  try {
    ...
    if (!relayBaseUrl) return buildAisStreamFallbackSnapshot();
    ...
    return buildAisStreamFallbackSnapshot();
  }
}
...
// AISStream fallback
//
// When the Railway relay is unavailable we cannot maintain the persistent
// WebSocket subscription that aisstream.io requires (Vercel Edge does not
// support outbound WebSocket clients). Instead we synthesize a snapshot from
// the chokepoint registry so the panel renders the canonical 13 chokepoints
// with reasonable baseline traffic intensities. This is clearly synthetic
// data — the `status.connected` flag is set to indicate the fallback path.
// AISSTREAM_API_KEY is read so that a future Railway / Supabase scheduled
// task can populate live density without code changes.
// ============================================================================
function buildAisStreamFallbackSnapshot(): VesselSnapshot {
  const hasKey = Boolean(process.env.AISSTREAM_API_KEY);
  const densityZones: AisDensityZone[] = CHOKEPOINT_REGISTRY.map((c) => ({
    id: c.id,
    name: c.displayName,
    ...
    deltaPct: 0,
    shipsPerDay: 0,
    note: hasKey ? 'AIS relay offline — showing chokepoint baseline' : 'AIS relay offline',
  }));
  return {
    snapshotAt: Date.now(),
    densityZones,
