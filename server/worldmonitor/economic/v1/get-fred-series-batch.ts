...
export async function getFredSeriesBatch(
  ...
  try {
    ...
    for (let i = 0; i < limitedList.length; i++) {
      ...
      const cached = entry?.status === 'fulfilled' ? (entry.value as { series?: FredSeries } | null) : null;
      if (cached?.series) {
        results[id] = applyFredObservationLimit(cached.series, limit);
      } else {
        missing.push(id);
      }
    }

    // Fallback: no Railway seed cache → call FRED API directly for missing IDs.
    if (missing.length > 0) {
      const live = await Promise.allSettled(
        missing.map((id) =>
          cachedFetchJson<{ series: FredSeries }>(
            fredLiveCacheKey(id),
            FRED_LIVE_FETCH_TTL,
            () => fetchFredSeriesLive(id),
          ),
        ),
      );
      for (let i = 0; i < missing.length; i++) {
        const id = missing[i]!;
        const entry = live[i];
        if (entry?.status === 'fulfilled' && entry.value?.series) {
          results[id] = applyFredObservationLimit(entry.value.series, limit);
        }
      }
    }
