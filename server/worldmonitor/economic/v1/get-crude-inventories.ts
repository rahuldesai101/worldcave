...
import { fetchEiaCrudeInventories } from './_eia-fallback';
...
export async function getCrudeInventories(
  ...
  try {
    ...
    if (!result?.weeks?.length) {
      const live = await fetchEiaCrudeInventories();
      if (live?.weeks?.length) return live as GetCrudeInventoriesResponse;
    ...
    const live = await fetchEiaCrudeInventories();
    return (live as GetCrudeInventoriesResponse) ?? { weeks: [], latestPeriod: '' };
  }
}
