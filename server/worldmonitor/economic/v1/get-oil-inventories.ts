...
import {
  fetchEiaCrudeInventories,
  fetchEiaNatGasStorage,
  fetchEiaSpr,
  fetchEiaRefineryInputs,
} from './_eia-fallback';
...
export async function getOilInventories(
  ...
  try {
    let [crudeRaw, sprRaw, natGasRaw, euGasRaw, ieaRaw, refineryRaw] = await Promise.all([
    ...
    // Live EIA fallback for the seed-cache misses (Railway seeders not deployed).
    const [crudeFb, natGasFb, sprFb, refineryFb] = await Promise.all([
      crudeRaw?.weeks?.length ? null : fetchEiaCrudeInventories(),
      natGasRaw?.weeks?.length ? null : fetchEiaNatGasStorage(),
      sprRaw?.barrels ? null : fetchEiaSpr(),
      refineryRaw?.inputsMbblpd ? null : fetchEiaRefineryInputs(),
    ]);
    if (crudeFb) crudeRaw = crudeFb as unknown as CrudeRaw;
    if (natGasFb) natGasRaw = natGasFb as unknown as NatGasRaw;
    if (sprFb) sprRaw = sprFb as unknown as SprRaw;
    if (refineryFb) refineryRaw = refineryFb as unknown as RefineryRaw;
