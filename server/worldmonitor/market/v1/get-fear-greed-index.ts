...
const LIVE_CACHE_KEY = 'market:fear-greed-altme-live:v1';
const LIVE_TTL = 3600; // alternative.me updates once per day.
...
async function fetchAlternativeMeLive(): Promise<{ score: number; previous: number } | null> {
  // alternative.me crypto F&G — public, no key. Used as a proxy when seed cache is empty.
  ...
  return {
    score: Number(arr[0]?.value ?? 0),
    previous: Number(arr[1]?.value ?? 0),
  };
}
...
export async function getFearGreedIndex(
  ...
  try {
    ...
    if (!raw?.composite) {
      const live = await cachedFetchJson<{ score: number; previous: number }>(
        LIVE_CACHE_KEY,
        LIVE_TTL,
        fetchAlternativeMeLive,
      );
      if (live && Number.isFinite(live.score)) {
        const empty: FearGreedCategory = { score: live.score, weight: 0, contribution: 0, degraded: true, inputsJson: '{}' };
        return {
          compositeScore: live.score,
          compositeLabel: labelFor(live.score),
          previousScore: live.previous,
          seededAt: new Date().toISOString(),
          sentiment: empty, volatility: empty, positioning: empty, trend: empty,
          breadth: empty, momentum: empty, liquidity: empty, credit: empty,
          macro: empty, crossAsset: empty,
          vix: 0, hySpread: 0, yield10y: 0, putCallRatio: 0, pctAbove200d: 0,
          cnnFearGreed: live.score, cnnLabel: labelFor(live.score),
          aaiiBull: 0, aaiiBear: 0, fedRate: '', fsiValue: 0, fsiLabel: '',
          hygPrice: 0, tltPrice: 0,
          sectorPerformance: [] as FearGreedSectorPerformance[],
          unavailable: false,
        };
      }
